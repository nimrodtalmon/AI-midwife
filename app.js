...
function renderCommitments() {
  if (state.commitments.length === 0) {
    commitmentsList.innerHTML = '<p class="text-xs text-gray-700 italic">None yet</p>';
    return;
  }
  ...
  state.commitments.forEach(c => {
    ...
    div.addEventListener('click', () => {
      if (!isLoading) dispatch({ type: 'retract', value: c });
    });
...
function showDraft() {
  if (state.artifact) {
    draftContent.innerHTML = renderMd(state.artifact);
  } else {
    draftContent.innerHTML = '<p class="text-gray-500 text-sm italic">Answer a few more questions, then try again.</p>';
  }
...
function setLoading(val) {
  ...
  document.querySelectorAll('.option-btn, #pass-btn, #skip-btn, #freetext-btn, #show-draft-btn')
